...
def test_cash_balance(test_account: int, expected_balance: float):
...
def test_holding_balance(test_account: int, expected_holding_balance: Dict[ID, float]):
    records = transaction_manager.get_all_for_account(
        test_account, until=BALANCE_AS_OF_DATE
    )
    holding_balances: Dict[ID, float] = defaultdict(lambda: 0.0)
